...
pub_two = rospy.Publisher('/depth_features', DepthFeatures, queue_size = 10)
...
def laserScanCB(data):
	...
	line_markers = buildRvizLineList(lines)
	corner_markers = buildRvizCorners(corners)
	...
	pub_two.publish(depthFeatures)
	pub.publish(corner_markers)
	pub.publish(line_markers)
def main():
	print("In Main")
	##initalize node
